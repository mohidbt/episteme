...
const buildReq = () =>
  new Request(`http://x/api/papers/${PAPER_ID}/auto-highlight`, {
    method: "POST",
    body: JSON.stringify({ instruction: "highlight all theorems" }),
  }) as unknown as import("next/server").NextRequest;
const routeParams = { params: Promise.resolve({ id: PAPER_ID }) };

beforeEach(() => vi.resetAllMocks());

describe("POST /api/papers/[id]/auto-highlight", () => {
  ...
  // GSD-132: managed bucket exhausted → 402 trial_exhausted (pre-stream).
