...
vi.mock("@/lib/citations/auto-link", () => ({
  autoLinkPaperCitations: vi.fn(async () => ({ linked: 0 })),
}));
...
describe("POST /api/papers/[id]/citations/rematch HMAC dual-auth", () => {
  it("401 when unauthenticated", async () => {
    vi.mocked(getAuthedUserId).mockResolvedValue(null);
    const { POST } = await import("./route");
    const res = await POST(
      new Request("http://localhost/api/papers/p1/citations/rematch", {
        method: "POST",
      }),
      ctx,
    );
