...
vi.mock("@/lib/db", () => {
  const orderByMock = () => Promise.resolve([]);
  return {
    db: {
      select: () => ({
        from: () => ({
...
vi.mock("@/lib/citations/enrich-refs", () => ({
  enrichRefsWithPaperMatchAndEdges: vi.fn(async (rows: unknown[]) => rows),
}));
...
describe("GET /api/papers/[id]/citations HMAC dual-auth", () => {
  it("401 when unauthenticated", async () => {
    vi.mocked(getAuthedUserId).mockResolvedValue(null);
    const { GET } = await import("./route");
    const res = await GET(
      new Request("http://localhost/api/papers/p1/citations"),
      ctx,
    );
