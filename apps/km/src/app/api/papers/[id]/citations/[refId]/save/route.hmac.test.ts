...
vi.mock("@/lib/db", () => ({
  db: {
    select: () => ({
      from: () => ({
        where: () => ({
          limit: () => Promise.resolve([{ id: "p1", userId: "u1", libraryId: 1 }]),
        }),
      }),
    }),
  },
...
describe("POST /api/papers/[id]/citations/[refId]/save HMAC dual-auth", () => {
  it("401 when unauthenticated", async () => {
    vi.mocked(getAuthedUserId).mockResolvedValue(null);
    const { POST } = await import("./route");
    const res = await POST(
      new Request("http://localhost/api/papers/p1/citations/1/save", {
        method: "POST",
  ...
  it("passes rawBody to HMAC verifier", async () => {
    const captured: { body?: string } = {};
    vi.mocked(getAuthedUserId).mockImplementation(async (req, body) => {
      captured.body = body;
    ...
    const { POST } = await import("./route");
    const payload = JSON.stringify({ folderId: "f1" });
    await POST(
      new Request("http://localhost/api/papers/p1/citations/1/save", {
        method: "POST",
        body: payload,
