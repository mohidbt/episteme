...
ALTER TABLE "document_references" ALTER COLUMN "authors" SET DATA TYPE jsonb USING CASE WHEN "authors" IS NULL THEN NULL ELSE "authors"::jsonb END;--> statement-breakpoint
ALTER TABLE "library_references" ALTER COLUMN "authors" SET DATA TYPE jsonb USING CASE WHEN "authors" IS NULL THEN NULL ELSE "authors"::jsonb END;--> statement-breakpoint
