...
async def invoke(req: Request, auth: InternalAuthDep):
    ...
    # strip the tokens from user text and surface their {kind,id,title} via a
    # system-preamble. Asset tokens (GSD-41) are processed afterwards on the
    # cleaned residue.
    cleaned_message, library_handles = parse_library_tokens(user_message)
    library_system_hint = build_library_system_hint(library_handles)
    ...
    user_content = await build_user_content(cleaned_message, user_id=user_id)
    ...
    if library_system_hint:
        input_messages.append({
            "role": "system",
            "content": library_system_hint,
        })
