---
name: paper-search
description: "Use when the user asks to find, search, locate, or download a paper PDF for a reference. Call agentic_search_papers(reference_id) first — it searches Semantic Scholar by DOI then by title/author/year and returns ranked candidates with match_confidence. Present candidate #1 to the user; on rejection present #2, etc. On approval call agentic_fetch_papers(reference_id, paper_url, metadata) to download the PDF and link it. Never use list_references or search_notes to find papers — always use agentic_search_papers."
...
3. If `found: false`, inform the user and suggest adding a DOI to the reference
