...
class SkillsBackend(BackendProtocol):
    ...
    def glob(self, pattern: str, path: str = "/") -> GlobResult:
        ...
        """
        # Strip leading slash so `_glob_match` operates on relative paths,
        # mirroring NotesBackend.aglob and FilesystemBackend.glob.
        ...
        # Apply search root (path) filter, then pattern match. The composite
        # passes "/" when routing through us, but be defensive for direct use.
        base = path.rstrip("/")
